...
var g1Distance = 200; //how far can goombas go
...
//timer
	...
	textSize(30);
	text('TIME REMAINING:', 600, 50);
	text(timeLimit2-gameTime2, 700, 50); //display countdown timer
...
//drawing and colliding with platforms
	...
	if(p1X >= c10X-cWidth/2 && p1X <= c10X+cWidth/2 && p1Y >= c10Y-cHeight/2 && p1Y <= c10Y+cHeight/2){
		//mario hits coin
		score2 = score2+1;//get point
		c10X = -1000; //move coin off screen
		coinSound.play();
	}//close hit coin
...
//code to trigger win or lose screens
	if(score2 >= 6){//4 to win because there are 6 coins on level 2
		winSound.play();
		stage = 2; //trigger win screen
	}//close you win
	
	if(lives2 <= 0){//no more lives so lose
		loseSound.play();
		stage = 3;
	}//close lose because no lives
		
	if(gameTime2 >= timeLimit2){//out of time in level 2
		loseSound.play();
		stage = 3;
	}//close lose because no time
...
function player1(){
	...
	if(lookingRight == true){//walking to the right
		lookingLeft = false;
		step = step+1; //walking
		if(step == 0){
			image(marioRight1, p1X, p1Y, pWidth, pHeight);
		}//close step 
		else if(step == 1){
			image(marioRight1, p1X, p1Y, pWidth, pHeight);
		}//close step
			...
			image(marioRight2, p1X, p1Y, pWidth, pHeight);
		}//close step
		else if(step == 4){
			image(marioRight2, p1X, p1Y, pWidth, pHeight);
		}//close step
	...
	if(lookingLeft == true){//walking to the left
		lookingRight = false;
		step = step+1; //walking
		if(step == 0){
			image(marioLeft1, p1X, p1Y, pWidth, pHeight);
		}//close step 
		else if(step == 1){
			image(marioLeft1, p1X, p1Y, pWidth, pHeight);
		}//close step
			...
			image(marioLeft2, p1X, p1Y, pWidth, pHeight);
		}//close step
		else if(step == 4){
			image(marioLeft2, p1X, p1Y, pWidth, pHeight);
		}//close step
	...
	if(lookingRight == false && lookingLeft == false && jump == false){//not walking and not jumping
			image(marioStill, p1X, p1Y, pWidth, pHeight); //still image
	}//close not walking and not jumping
	else if(lookingRight == false && lookingLeft == false && jump == true){ //jumping not walking
		image(marioJump, p1X, p1Y, pWidth, pHeight);//jump image
	}//close jumping
